# Azure Windows Web App Slot with .NET and Datadog Monitoring

This example demonstrates how to deploy a Windows-based Azure Web App **deployment slot** running .NET 9.0 with Datadog monitoring enabled.
...
- A parent Azure Web App configured for .NET 9.0 with Datadog monitoring
...
The example includes a local code deployment step that packages and deploys the sample .NET application from the `src` directory to the staging slot.
