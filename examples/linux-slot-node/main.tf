...
module "datadog_linux_web_app_slot" {
  source          = "../../modules/linux-slot"
  datadog_api_key = var.datadog_api_key
  datadog_site    = var.datadog_site
  datadog_env     = "dev"
  datadog_service = "my-service"
  ...
  app_service_id = module.datadog_linux_web_app.id
  name           = "staging"
  https_only     = true
  site_config = {
    application_stack = {
...
resource "terraform_data" "code_deployment" { # Basic local deployment setup, replace with your actual deployment method in prod
  depends_on = [module.datadog_linux_web_app_slot]
  provisioner "local-exec" {
    command = <<EOT
    zip code.zip index.js package.json
    az webapp deploy -g ${azurerm_resource_group.example.name} -n ${module.datadog_linux_web_app.name} --src-path code.zip --type zip --slot staging
    EOT
  }
}
